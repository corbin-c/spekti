...
let SpektiEntity = class {
  ...
  async getContent() {
    let content = await this.gist.getContent(file);
    console.info("retrieved "+file+" from gist");
    this.content = JSON.parse(content);
    this.ready();
  }
  async commitChanges() {
    await this.gist.putContent(file,JSON.stringify(this.content));
  }
  get allContent() {
...
let Spekti = class {
  ...
  async init() {
    ...
    if (gist_list.filter((e) => (e.description == GIST_DESC)).length == 0) {
      ...
      await Promise.all(this.entities.slice(1).map(async (entity) => {
        await this.gist.putContent(entity.filename,"[]");
      }));
    } else {
    ...
    this.entities = this.entities.map(e => {
      return new ENTITIES[e.type](this.gist,e.filename);
    })
    this.isReady();
  }
}
